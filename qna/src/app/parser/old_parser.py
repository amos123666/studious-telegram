...
def old_parseQuestionsAnswersFromFile(filePath: str, target_model: str):
    ...
    '''
    threads = old_parseThreadsFromFile(filePath)
    file = old_getPostsFromThreads(threads, target_model)
    json = JsonLoader(file)
    return json.read_data()


def old_parseThreadsFromFile(filePath: str):
    '''
...
def old_getPostsFromThreads(threads, target_model):
    '''
...
def old_get_summarisation(data, tokenizer, model):
